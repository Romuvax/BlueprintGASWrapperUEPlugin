...
{
	PluginName = TEXT("BlueprintGASWrapper");
...
{
	const TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("BlueprintGASWrapper"));
	if (Plugin.IsValid())
	{
...
bool UGSCAttributeSetClassTemplate::PrepareHeaderTemplate(TArray<FGSCAttributeDefinition> Attributes, FString TemplateText, FText& OutFailReason)
{
	...
	if (!bReplaceDone)
	{
		UE_LOG(LogBlueprintGASWrapper,Error, TEXT("ClassTemplate:PrepareHeaderTemplate() Error replacing attribute placeholders: %s"), *OutFailReason.ToString())
		return false;
	}

	const FString OutputFilename = GetPluginTemplateDirectory() / TEXT("AttributeSetClass.h.template");

	UE_LOG(LogBlueprintGASWrapper, Display, TEXT("ClassTemplate:PrepareHeaderTemplate() Write Output to: %s"), *OutputFilename)
...
bool UGSCAttributeSetClassTemplate::PrepareSourceTemplate(TArray<FGSCAttributeDefinition> Attributes, FString TemplateText, FText& OutFailReason)
{
	...
	if (!bReplaceDone)
	{
		UE_LOG(LogBlueprintGASWrapper, Error, TEXT("ClassTemplate:PrepareSourceTemplate() Error replacing attribute placeholders: %s"), *OutFailReason.ToString())
		return false;
	}

	const FString OutputFilename = GetPluginTemplateDirectory() / TEXT("AttributeSetClass.cpp.template");

	UE_LOG(LogBlueprintGASWrapper, Display, TEXT("ClassTemplate:PrepareSourceTemplate() Write Output to: %s"), *OutputFilename)
	return WriteOutputFile(OutputFilename, TemplateText, OutFailReason);
}
...
{
	UE_LOG(LogBlueprintGASWrapper, Verbose, TEXT("ClassTemplate:ReplaceAttributePlaceholder() Alter template here"))
...
FString UGSCAttributeSetClassTemplate::MakeAttributesDeclaration(TArray<FGSCAttributeDefinition> InAttributesList)
{
	...
	{
		UE_LOG(LogBlueprintGASWrapper, Error, TEXT("ClassTemplate:MakeAttributesDeclaration() Error reading template: %s"), *FailReason.ToString())
		return FString();
	}
...
FString UGSCAttributeSetClassTemplate::MakeAttributesOnRepDeclaration(TArray<FGSCAttributeDefinition> InAttributesList)
{
	...
	{
		UE_LOG(LogBlueprintGASWrapper, Error, TEXT("ClassTemplate:MakeAttributesOnRepDeclaration() Error reading template: %s"), *FailReason.ToString())
		return FString();
	}
...
FString UGSCAttributeSetClassTemplate::MakeAttributesDoRepLifetimeDefinition(TArray<FGSCAttributeDefinition> InAttributesList)
{
	...
	{
		UE_LOG(LogBlueprintGASWrapper, Error, TEXT("ClassTemplate:MakeAttributesDoRepLifetimeDefinition() Error reading template: %s"), *FailReason.ToString())
		return FString();
	}
...
FString UGSCAttributeSetClassTemplate::MakeAttributesOnRepDefinition(TArray<FGSCAttributeDefinition> InAttributesList)
{
	...
	{
		UE_LOG(LogBlueprintGASWrapper, Error, TEXT("ClassTemplate:MakeAttributesOnRepDefinition() Error reading template: %s"), *FailReason.ToString())
		return FString();
	}
