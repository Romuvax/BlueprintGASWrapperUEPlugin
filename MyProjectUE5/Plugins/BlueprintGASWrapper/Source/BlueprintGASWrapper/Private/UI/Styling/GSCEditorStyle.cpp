...
{
	static FName StyleSetName(TEXT("BlueprintGASWrapperStyle"));
	return StyleSetName;
}
...
{
	TSharedRef<FSlateStyleSet> Style = CreateStyle("BlueprintGASWrapperStyle");
	Style->Set("BlueprintGASWrapperStyle.OpenPluginWindow", new IMAGE_BRUSH(TEXT("GSC40x40"), Icon40x40));
	Style->Set("BlueprintGASWrapperStyle.OpenPluginWindow.Small", new IMAGE_BRUSH(TEXT("GSC20x20"), Icon20x20));
...
TSharedRef<FSlateStyleSet> FGSCEditorStyle::CreateStyle(const FName StyleSetName)
{
	...
	Style->SetContentRoot(IPluginManager::Get().FindPlugin("BlueprintGASWrapper")->GetBaseDir() / TEXT("Resources"));
	return Style;
}
